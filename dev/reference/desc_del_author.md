...
It uses the Authors@R field. The author(s) to be removed can be
specified via any field(s). All authors matching all specifications will
be removed. E.g. if only `given = "Joe"` is supplied, then all authors
whole given name matches `Joe` will be removed. The specifications can
be (PCRE) regular expressions.
