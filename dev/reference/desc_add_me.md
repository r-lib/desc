...
`desc_add_me` is a convenience function, it adds the current user as an
...
environment variable in .Renviron. The full name is parsed using
`as.person` and collapsing the given name and the family name in order
to e.g. have the first and middle names together as given name. This
approach might be limited to some full name structures.
